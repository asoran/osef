...
var textureF = PIXI.Texture.fromImage('assets/bunnyFire.png');
var carrotTex = PIXI.Texture.fromImage('assets/carrot.png');

// create a new Sprite using the texture
var bunny = new PIXI.Sprite(texture);
var bunnyF = new PIXI.Sprite(textureF);
...
bunny.hitArea = new PIXI.Rectangle(0, 0, 20, 20);

bunnyF.position.x = 500;
bunnyF.position.y = 200;
bunnyF.hitArea = new PIXI.Rectangle(0, 0, 20, 20);
...
function animate() {  
  ...
  for(var b=bullets.length-1;b>=0;b--){
    ...
    if(outOfScreen(bullets[b])){
      ...
      if(checkIfHitEnnemy(bullets[b], bunnyF)) {
        bunnyF.destroy();
        bunnyF = undefined;
        bullets[b].destroy();
        bullets.splice(b, 1);}
...
function checkIfHitEnnemy(a: any, b: any) {
  var ab = a.getBounds();
  var bb = b.getBounds();
